...
import { DEFAULT_STOREFRONT_NAME, safeJsonLd, siteUrl } from '@/lib/storefront'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const description = category.meta_description || category.description ||
        `Shop ${category.name.toLowerCase()} products from trusted brands at ${DEFAULT_STOREFRONT_NAME}.`
    return {
...
export default async function CategoryPage({ params, searchParams }: Props) {
    ...
    const schema = {
        ...
        url: `${canonicalOrigin}/shop/categories/${category.slug}`,
        isPartOf: { '@type': 'WebSite', name: storefront.name, url: canonicalOrigin },
    }
    return (
