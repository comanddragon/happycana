...
export function Logo({
    ...
    return (
        <Link href={href} className="group flex shrink-0 items-center gap-2.5 text-hc-paper" aria-label={`${storefront.name} home`}>
            ...
            <span className="leading-none"><span className="block font-hc-display text-xl font-medium tracking-[.01em]">{storefront.name}</span><span className="mt-1 hidden font-hc-mono text-[8px] uppercase tracking-[.22em] text-hc-sage sm:block">{storefront.kind === 'peptides' ? 'Research catalog' : storefront.kind === 'hash' ? 'Solventless archive' : 'Botanical dispensary'}</span></span>
        </Link>
    )
}
