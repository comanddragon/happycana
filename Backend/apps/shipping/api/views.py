...
from .serializers import (
    ...
    ShippingMethodSerializer, TrackingEventSerializer,
)
...
class ShipmentDetailView(generics.RetrieveUpdateAPIView):
    ...
    def get_serializer_class(self):
        return ShipmentUpdateSerializer if self.request.method in ("PUT", "PATCH") else ShipmentSerializer
...
class OrderShipmentListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if not self.request.user.is_staff:
            ...
            try:
                Order.objects.get(id=order_pk, user=self.request.user)
