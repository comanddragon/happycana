...
class IsParticipant(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if isinstance(obj, ChatMessage):
            return obj.room.customer_id == user.id or obj.room.agent_id == user.id
        return False
...
class ChatRoomViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = (
            for_request(ChatRoom.objects, self.request)
            .select_related("customer", "agent", "order")
            .prefetch_related(
                Prefetch(
                    "messages",
                ...
                Prefetch(
                    "messages",
                    queryset=ChatMessage.objects.filter(is_read=False).exclude(
                        sender=user
                    ),
