...
class PaymentListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return for_request(
            Payment.objects.select_related("order").order_by("-created_at"),
            self.request,
            "order__storefront",
        )
...
class PaymentDetailView(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        return for_request(
            Payment.objects.select_related("order"), self.request, "order__storefront"
        )
...
class InitiatePaymentView(APIView):
    ...
    def post(self, request, order_pk):
        ...
        order = for_request(Order.objects, request).get(pk=order_pk, user=request.user)
...
@method_decorator(
    ratelimit(key="ip", rate="60/m", method="POST", block=True), name="dispatch"
)
...
class CapturePayPalPaymentView(APIView):
    ...
    def post(self, request, order_pk):
        ...
        if not gateway_ref:
            return Response(
                {"detail": "gateway_ref is required."},
