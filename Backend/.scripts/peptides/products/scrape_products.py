...
The catalog is rendered client-side by JavaScript - a plain HTTP GET only
returns the page shell/navigation, with no product cards in the markup.
This scraper therefore drives a real headless browser (Playwright) instead
of using `requests`, so it sees the same DOM a real visitor's browser would
...
MAX_RETRIES = 5
BACKOFF_SECONDS = 2.0
# PeptidesDirect renders its catalog client-side (the document returned by
# a plain HTTP GET only contains the shell/navigation - product cards are
# injected by JavaScript after load). A `requests` GET can never see them,
# which is why every product count came back as 0. We drive a real headless
# browser instead and read the DOM back out after it has rendered.
...
class BrowserSession:
    ...
    def get(self, url, wait_selector=PRODUCT_LINK_SELECTOR):
        self._page.goto(
            url,
        ...
        if wait_selector:
            try:
                ...
                # The page may legitimately have no matches (e.g. an
                # empty category) - downstream parsing will just report
                # 0 either way, so don't treat this as fatal here.
                pass
        # Let any late XHRs / lazy images settle before reading the DOM.
...
def get(session, url, wait_selector=PRODUCT_LINK_SELECTOR):
    for attempt in range(MAX_RETRIES + 1):
        try:
            return session.get(url, wait_selector=wait_selector)
