...
Status update (2026-09-01): Recommended next steps 1-6 below have been implemented — see "Shipped" notes inline and the summary at the bottom. Still open: Google Business Profile / local business details (a business/ops question, not a code change) and a static robots.txt (a dynamic app/robots.ts already covers this, so it's not urgent).
...
2. Trust/compliance claims aren't backed by verifiable content — ✅ Shipped
...
Also shipped: a dedicated backend endpoint (GET /api/catalog/labs/) lists every variant with a real, on-file COA, and a new /learn/lab-results page renders that list — one crawlable page that aggregates every batch's certificate, linked from the homepage's LabTrust section and from the what-is-a-cannabis-coa guide. There's still no license-number detail page, which remains a minor, lower-priority gap.
...
6. Make the lab-testing claims verifiable: link each product's real COA (even a simple PDF/image per batch) instead of only a badge graphic, and consider a small "Lab Results" index page. — ✅ Shipped, see issue #2. Per-product COA linking already existed; the homepage's fabricated batch data has been replaced with real product data, and a new dedicated backend endpoint plus /learn/lab-results page now aggregates every on-file certificate in one place.
...
- Backend/apps/catalog/managers.py — new ProductVariantQuerySet/Manager.with_coa()
- Backend/apps/catalog/api/serializers.py — new LabResultProductSerializer, LabResultSerializer
- Backend/apps/catalog/api/views.py — new LabResultListView
- Backend/apps/catalog/api/urls.py — new GET /api/catalog/labs/ endpoint
- frontend/src/types/index.ts — new LabResult / LabResultProduct types
- frontend/src/lib/catalog.server.ts — new getLabResults() fetcher
- frontend/src/app/learn/lab-results/page.tsx — new, lists every product with a real COA on file
- frontend/src/components/home/LabTrust.tsx — links to /learn/lab-results
- frontend/src/app/learn/what-is-a-cannabis-coa/page.tsx — "Where to see ours" now links to /learn/lab-results
- frontend/src/app/sitemap.ts — added /learn/lab-results
